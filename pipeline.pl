...
if ((!$input or !$output or !$genomeType or !$runID) and $cd){
	`clear`;
	print <<HelpDocumentation;

DESCRIPTION:
	In order to run cuffdiff using this pipeline, all samples must already have been
	processed through the rna_seq_pipeline.pl pipeline (or at least through the cuffquant step).

	Because cuffdiff can significantly add to the runtime of the pipeline, it has been
	added here as an option for those who wish to deeply analyze differential expression.

	Notice that the only difference in the command is that the input file path has been modified.
	The input file path must specify the "cq-out" directory generated by the rna_seq_pipeline.pl call.
	Alternatively, if only a subset of samples are to be analyzed for differential expression,
	move the appropriate folders in the "cq-out" elsewhere and direct the input argument to their new location.

USAGE:
	perl pipeline.pl -i <INPUT> -o <OUTPUT> -g <GENOME> -r <RUNID> <OPTIONS> --cd

EXAMPLE RUN:
	perl pipeline.pl -i /mnt/state_lab/share/rna_seq_out/cq-out/ -o /mnt/state_lab/share/Manoj/rna_seq_out/ -g u -r 81214 --cd
HelpDocumentation
	exit;
}
