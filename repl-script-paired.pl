...
for (my $i=1; $i <= $replicates; $i++) {
	...
	print SUB "SAMPLE_LIST_N".$i."_R2=(\$SAMPLE_DIR/*_N".$i."_R2.\$ARG6);\n";
}
...
for (my $i=1; $i <= $replicates; $i++) {
	...
	print SUB "INPUT_FILE_N".$i."_R2=\${SAMPLE_LIST_N".$i."_R2[\$INDEX]};\n";
}
...
for (my $i=1; $i <= $replicates; $i++) {
	...
	$input_R2 = $input_R2 . "\$INPUT_FILE_N".$i."_R2,";
}

$input_R1 =~ s/.$// if (substr($input_R1, -1, 1) eq ",");
$input_R2 =~ s/.$// if (substr($input_R2, -1, 1) eq ",");

print SUB <<ShellScript;
#Script to execute
perl run_pipeline.pl -i $input_R1 -o \$ARG2 -g \$ARG3 -p \$ARG4 -r \$ARG5 -m \$ARG7 -n \$ARG8 -e $input_R2
ShellScript
