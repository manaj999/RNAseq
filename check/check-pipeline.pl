...
if ($part == 1) {
	...
	print LOG "doing this..\n";
	print LOG "/opt/sge625/sge/bin/lx24-amd64/qsub -l h=ihg-node-27 -t $t -tc $tc -v ARG1=$input,ARG2=$output,ARG3=$genomeType,ARG4=$part,ARG5=$runID,ARG6=$suffix,ARG7=$merge,ARG8=$novel check-submit-1.sh\n\n";
	#`/opt/sge625/sge/bin/lx24-amd64/qsub -t $t -tc $tc -v ARG1=$input,ARG2=$output,ARG3=$genomeType,ARG4=$part,ARG5=$runID,ARG6=$suffix,ARG7=$merge,ARG8=$novel -S /bin/sh check-submit-1.sh`;
	if ($paired){
		`/opt/sge625/sge/bin/lx24-amd64/qsub -t $t -tc $tc -v ARG1=$input,ARG2=$output,ARG3=$genomeType,ARG4=$part,ARG5=$runID,ARG6=$suffix,ARG7=$merge,ARG8=$novel -S /bin/sh check-submit-paired-1.sh`;
	}
	else {
		`/opt/sge625/sge/bin/lx24-amd64/qsub -t $t -tc $tc -v ARG1=$input,ARG2=$output,ARG3=$genomeType,ARG4=$part,ARG5=$runID,ARG6=$suffix,ARG7=$merge,ARG8=$novel -S /bin/sh check-submit-1.sh`;
	}
	...
	unless($overrideCM) {
		...
		`perl check-run_pipeline.pl -i $input -o $output -g $genomeType -p $part -r $runID -m $merge --cm`;
	}	
}
elsif ($part == 3){
	...
	`/opt/sge625/sge/bin/lx24-amd64/qsub -t $t -tc $tc -v ARG1=$output,ARG2=$output,ARG3=$genomeType,ARG4=$part,ARG5=$runID,ARG6=$suffix,ARG7=$merge,ARG8=$novel -S /bin/sh check-submit-2.sh`;
}
elsif ($part == 4){

	# After all samples have been run through cuffquant in Step 2, submit cuffnorm job to Sun Grid Engine
	`perl check-run_pipeline.pl -i $input -o $output -g $genomeType -p $part -r $runID -m $merge -n $novel`;
}
...
if ($cd){
	`/opt/sge625/sge/bin/lx24-amd64/qsub -pe parallel 8 -V -S /usr/bin/perl check-run_pipeline.pl -i $input -o $output -g $genomeType -r $runID -m $merge -n $novel --cd`;
}
close(LOG);
